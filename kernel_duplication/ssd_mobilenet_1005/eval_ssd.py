...
parser.add_argument('--recover_type', default='KR', type=str, help='Type of recovery: KR & FMR')
...
if __name__ == '__main__':
    ...
    net.recover_type = args.recover_type
    ...
    if not args.duplicated:
        ...
        if args.ft_type == "attention":
            ...
            for k in net.all_layer_indices:
                ...
                final = final.sort(dim=1, descending=True)
                net.all_duplication_indices[k] = final.indices[0]
        elif args.ft_type == "entropy":
            print("entropy:")
            weights_imp = copy.deepcopy(net.state_dict())
            net_imp.load_state_dict(weights_imp)
            net_imp.error = 0
            net_imp.weights_error = 0
            net_imp.entropy_flag = True
            cal_entropy(net_imp, config)
            exit()
    ...
    net.weights_error = args.weight_error
