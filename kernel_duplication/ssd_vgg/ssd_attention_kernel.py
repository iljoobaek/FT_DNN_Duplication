...
class Mask(object):
    ...
    def __init__(self, width, active_width):
        self.width = width
        self.active_width = active_width
        data = torch.randn(width)
        self.mask = torch.nn.Parameter(data)
        self.softmax = nn.Softmax()

    def __call__(self, x):
        self._normal()
        self.mask.requires_grad = True
        print(self.mask.size(), x.size())
        print(type(self.mask), type(x))
        return self.mask * x

    def _normal(self):
        self.mask = self.softmax(self.mask)
        self.mask = self.active_width * self.mask
...
class SSD(nn.Module):
    ...
    def __init__(self, args, phase, size, base, extras, head, num_classes):
        ...
        # self.mask = Mask(512, 256)
        # self.mask = torch.nn.Parameter(torch.randn(512), requires_grad=True)
        value = torch.zeros(512)
        value[torch.randperm(512)[:256]] = 1
        self.mask = torch.nn.Parameter(value, requires_grad=True)
        self.softmax_mask = nn.Softmax()
