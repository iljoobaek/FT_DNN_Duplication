...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc1(flat_x)
        out.retain_grad()
        output.append(out) # check if output needs flat layer?
        out = self.fc2(out)
        out.retain_grad()
        output.append(out)
        return out, output
...
# D2NN: propagation evaluation
def propagation_eval(model):
    weights = model.state_dict()
    evaluation, names = weight_sum_eval(model)
...
def main():
    ...
    for epoch in range(args.epochs):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if cnt % args.log_every == 0:
                ...
                train_acc = correct/args.batch_size
    ...
    cal_importance(model, train_loader)
